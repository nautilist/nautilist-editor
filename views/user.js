...
function view (state, emit) {
  ...
  function renderUserProjects(){
    ...
    return projs.map(proj => {
      return html`
        <li class="mw6 shadow-5">
        <a class="link black" href="/projects/${proj._id}">
          <div class="flex pa3 bg-light-gray flex flex-row items-center">
            <div class="h2 w2 mr4" style="background-color:${proj.colors[proj.selectedColor]}"></div>
            <div class="flex flex-column">
            <h3 class="ma0">${proj.name}</h3>
            <p class="ma0 truncate">${proj.description}</p>
            </div>
          </div>
        </a>
        </li>
