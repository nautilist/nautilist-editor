...
class CodeEditor extends Component {
  ...
  createElement () {
    const cm =  html`<textarea class="h-100 w-100 pa0 ma0" name="code" id="code">${this.state.workspace.md}</textarea>`;
  ...
  load(el){
    ...
    this.editor.on('change',(cMirror) => {
      ...
      this.emit(this.state.events.workspace_md_update, cMirror.getValue())
    });
